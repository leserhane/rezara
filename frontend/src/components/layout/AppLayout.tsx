      ...
      <aside className="hidden w-64 shrink-0 flex-col border-r border-sand-200 bg-white lg:flex dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center gap-2.5 border-b border-sand-100 px-5 py-5 dark:border-slate-800">
          ...
          <div>
            ...
            <div className="text-xs text-sand-700 dark:text-slate-400">Gestion</div>
          </div>
        </div>
    ...
    <nav className="flex-1 space-y-4 overflow-y-auto px-3 py-2">
      {sections.map((section) => (
        <div key={section.label}>
          <div className="px-3 pb-1 text-[11px] font-semibold uppercase tracking-wide text-sand-700 dark:text-slate-500">{section.label}</div>
                ...
                className={({ isActive }) =>
                  clsx(
                    ...
                    isActive
                      ...
                      : 'text-slate-600 hover:bg-sand-100 dark:text-slate-300 dark:hover:bg-slate-800'
                  )
                }
              >
                <item.icon size={18} />
...
function ThemeToggle() {
  ...
  return (
    <div className="hidden items-center gap-0.5 rounded-lg border border-sand-200 p-0.5 sm:flex dark:border-slate-700">
      {options.map((o) => (
        <button
          key={o.key}
...
function UserMenu() {
  ...
  return (
    <div className="relative border-t border-sand-200 p-3 dark:border-slate-800">
      <button onClick={() => setOpen((o) => !o)} className="flex w-full items-center gap-3 rounded-lg p-2 hover:bg-sand-100 dark:hover:bg-slate-800">
      ...
      {open && (
        <div className="absolute bottom-full left-3 right-3 mb-1 rounded-lg border border-sand-200 bg-white p-1 shadow-lg dark:border-slate-700 dark:bg-slate-800">
          <button
            onClick={signOut}
